<!-- input: Prism Web 的运行方式、功能边界、探索策略、配置和公开 API -->
<!-- output: 项目级开发、部署、限次/持续探索与脚本维护说明 -->
<!-- position: 仓库总览和使用入口 -->
...
- **探索测试** — 主动调用 Agent 探索 Web 后台，支持动作上限或持续模式与可选时长，默认只读并可转 GitLab Bug 草稿
...
设置页配置保存在 `data/config.json`；没有 Web 配置时会回退到环境变量。GitLab 脚本提交可通过 `.env` 配置 `GITLAB_BASE_URL`、`GITLAB_PROJECT_PATH`、`GITLAB_BRANCH` 和 `GITLAB_TOKEN`，环境变量优先生效。
...
│   ├── exploration/       # 主动 Web AI 探索、历史与安全策略
│   ├── pi/                # PI Agent 集成
...
| `/api/scripts/gitlab-config` | GET | 读取 GitLab 脚本发布配置，不返回 token 明文 |
| `/api/scripts/publish-gitlab` | POST | 提交稳定脚本包到 GitLab |
...
| `/api/stop` | POST | 停止执行 |
| `/api/exploration/run` | POST | 启动一次 Web AI 探索并返回 SSE 过程 |
| `/api/exploration/runs` | GET | 查询探索历史 |
| `/api/exploration/runs/:id/stop` | POST | 停止指定探索任务 |
...
# 探索执行器核心规则测试
node --test server/exploration/runner.test.js
```
